"""Rewrite selected Cast(... -> int64) nodes to Cast(... -> int32).

This targets index tensors whose consumers accept int32 as well as int64. The
rewrite preserves integer conversion semantics while halving that tensor's
official-static memory footprint. Candidates are still validation-gated because
ONNX checker is looser than ONNX Runtime kernel support for some ops.
"""
...
def _consumer_ops(graph: onnx.GraphProto, value_name: str) -> list[str]:
    ops: list[str] = []
...
def best_int64_cast_to_int32(
    source_model: str,
    output_model: str,
    min_cast_output_bytes: int = 64,
    max_cast_candidates: int = 0,
) -> dict[str, Any]:
    """Write the cheapest valid single int64-Cast-to-int32 rewrite."""
    ...
    for index, node in enumerate(source_model_proto.graph.node):
        ...
        if memory < min_cast_output_bytes:
            continue
        # Keep this broad but avoid known shape-only ops where int64 is often
        # required by schema or runtime.
        consumer_ops = set(_consumer_ops(source_model_proto.graph, node.output[0]))
    ...
    for _memory, index in cast_indices:
        ...
        _set_cast_to(model.graph.node[index], TensorProto.INT32)
                ...
                row.update(
                    {
                        ...
                        if changed
                        else "no cheaper int64 Cast to int32 rewrite found",
                    }
                )
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="outputs/candidates/int64_cast_to_int32")
    parser.add_argument("--report", default="outputs/reports/int64_cast_to_int32.csv")
    parser.add_argument("--lanes", default="overrides")
    parser.add_argument("--task-ids", default="")
    parser.add_argument("--min-cast-output-bytes", type=int, default=64)
